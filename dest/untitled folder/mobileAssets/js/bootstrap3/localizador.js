 ...
 var lati;
 var longi;
var abrirSinLocalizador = true;
var clickEnEstado = false;
...
$('.obtain-state').on('click', function() {
     ...
     clickEnEstado = true;
     console.log("Click en estado"+clickEnEstado);
...
$("#map-container").css("pointer-events", "none");
     ...
     estadoVar = $(this).find("h4").html();
      ...
      $.getJSON( "tiendas-format.json", function( data ) {
       ...
       });          $("#list-store .panel .list-group").append("<li class='list-group-item' >&nbsp;</li>");
...
registrandoPosicion = true;
    ...
    setMarkers(pos, radius, map);
$('input[name=optionsRadios]').trigger( "change" );
$("#map-container").css("pointer-events", "auto");
...
//directionsDisplay = new google.maps.DirectionsRenderer(rendererOptions);
  if(abrirSinLocalizador == true){
    openAllListTwo();
    abrirSinLocalizador = false;
     $("#map-container").css("pointer-events","none");
  }
  map = new google.maps.Map(document.getElementById('map-container'),
      mapOptions);
...
//$("#map-container").css("width","100%");
 ...
 function openAllList(){
  ...
  $(".modal-content-estate").animate({ scrollTop: 0 }, "slow");
...
closedAllOver();
  if ( $("#detalle-tienda").is(":visible") || $("#sectionAllLocators").is(":visible")) {
    console.log("pepejdbfjan");
   openAllMaps();

   } else { 
    //alert("no esta visible");
    console.log("lollollslls");
    window.history.back();
  }
...
$(".modal-content-estate").animate({ scrollTop: 0 }, "slow");
console.log("Ver el estatus"+statusBackForList);


if($('#myModal').hasClass('in') == true){

    $('#myModal').modal('toggle');
    console.log("entro el modal de filtros");

}else{



          if(clickEnEstado == true){
             ...
             statusBackForList = true;
                 ...
                 if($('#myModal').hasClass('in') == true){
                         ...
                         statusBackForList = false;
                         $('#myModal').modal('toggle');
                         console.log("cerrando filtros"+statusBackForList);
