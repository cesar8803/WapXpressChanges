...
$(document).ready(function() {	
	...
	$.ajax({
		...
		for(var i=0;i<Object.keys(data.StoreNames).length;i++){
			...
			suggestions[i]=Object.keys(data.StoreNames)[i];
...
function codeLatLng1(){ 	
	...
	var latlng;
	if(inputval!='' & suggestions.length > 1){
		for(var i=0;i<suggestions.length;i++){ 
			if(suggestions[i]==inputval){ 
				lngval=lng[i];
				latval=lat[i];  
			}
		...
		if (strUser === "dutyfree") mapIcon = "/assets/utils/almacenes_gm/images/df.png";
	    else if (strUser === "ff") mapIcon = "/assets/utils/almacenes_gm/images/ff.png";
	    else if (strUser === "liverpool") mapIcon = "/assets/utils/almacenes_gm/images/icoBolsa.png";
	    else if (strUser === "cc") mapIcon = "/assets/utils/almacenes_gm/images/cc.png";
		var contextPath = $("#storePage_contextPath").val();			 
...
function getOptions_onload(){
	...
	jQuery.get(action,function(data){
		...
		mapIcon = "/assets/utils/almacenes_gm/images/icoBolsa.png";
	    if (defLatlng != '') codeLatLng(defLatlng, mapIcon);
	    sortStores();
	});
}
...
function sortStores() {
	...
	var arStore = [];
	var i = 0;
	$('#lstAlmacenes').children().each(function() {
		arStore[i] = [ $(this).html(), $(this).attr('value') ]
		i++;
	});
	arStore.sort(function(a, b) {
		return a == b ? 0 : a < b ? -1 : 1;
	})
	$('#lstAlmacenes').children().remove();
	for (i = 0; i < arStore.length; i++) {
		$('#lstAlmacenes').append(
				'<option value="' + arStore[i][1] + '">' + arStore[i][0]
						+ '</option>');
	}
	arStore.length = 0;
	i = 0;
