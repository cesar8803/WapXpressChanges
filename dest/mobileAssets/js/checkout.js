...
function goToPromotionPage() {
	...
	if(isLoggedInId == 'true'){
		if ($('#form_checkout_express_pay_method').valid()) {
			var $loading = $('#sc-page-spinner').hide();
			$loading.show();
...
$(document).ready(function(){
	...
	/*var divMethodNull = $("#payment-container > div.panel-body.body-payment > ul > li > span.payment_type");
	if(divMethodNull != null){
		if($('#payment-container > div.panel-body.body-payment > ul > li > span.payment_type:contains("Pago PayPal")')){
			$("#address-container > div.panel-body.body-address.no-border-bottom > div:nth-child(3) > div:nth-child(2) > span.num-title").text("Folio paypal:");
		}
